...
namespace CalDav
{
    public static class Common
    {
        ...
        public static DateTime? ToDateTime(this string value, CalDAVCalendar calendar, string TimeZoneID)
        {
            var date = ToDateTime(value);
            if (date == null) return null;
            if (calendar == null) return date;

            var timeZones = calendar.TimeZones as ICollection<TimeZone>;
            var info = timeZones.SelectMany(x => x).Where(x => x.ID == TimeZoneID);
        ...
        internal static bool Property(this System.IO.TextReader rdr, out string name, out string value, XNameValueCollection parameters)
        {
            ...
            while (line.Length > 0) {
                ...
                if (name == null) {
                    ...
                    if (!part.Equals(name)) // TODO: test it.
                    {                       //
                        value += part;      // I added this lines for fix a bug with reading a DESCRIPTION 
                        value += sep;       // text with a time value like 19:00 (problem was with ':' symbol)
                    }
