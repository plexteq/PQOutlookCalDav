...
namespace CalDav.Client {
	public class RemoteCalendar : IRemoteCalendar {
        ...
        public string FullName
        {
            get
            {
                return string.Format("{0} ({1})", Name, Credentials.UserName.Replace('.','_'));
            }            
        }

        public RemoteCalendar(Common common)
        {
            this.common = common;
        }

        public RemoteCalendar(Common common, Uri Url, NetworkCredential credentials)
        {
            this.Url = Url;
            this.common = common;
		...
		public CalendarCollection Search(CalDav.CalendarQuery query) {
            XElement q = (XElement)query;

            var result = common.Request(Url, "REPORT", q, Credentials, new Dictionary<string, string> {
				{ "Depth", "1" }
			});
			var xdoc = XDocument.Parse(result.ResponseContent);
	    ...
	    public IEvent GetEvent(string uid)
	    {
