...
namespace CalDav.Client {
	public class Server : IServer {
        ...
        public RemoteCalendar GetCalendar(string name)
        {
            var id = NameToId(name);
            ...
            List<RemoteCalendar> calendars = new List<RemoteCalendar>();
            ...
            foreach (XElement response in responses)
            {
                if (response.Descendants(CalDav.Common.xCalDav.GetName("calendar")).Count() > 0)
                {
                    ...
                    calendars.Add(new RemoteCalendar(_common, new Uri(Url, href), Credentials));
                }
            }
        ...
        public RemoteCalendar[] GetCalendars() {
            var result = _common.Request(Url, "propfind", new XDocument(
                    new XElement(CalDav.Common.xDav.GetName("propfind"),
                        new XElement(CalDav.Common.xDav.GetName("prop"),
            ...
            if (string.IsNullOrEmpty(result.ResponseContent))
				return new[]{
					 new RemoteCalendar(_common) { Url =  Url, Credentials = Credentials }
				};
        ...
        IRemoteCalendar[] IServer.GetCalendars()
        {
            return GetCalendars();
        }

        public void CreateCalendar(IRemoteCalendar calendar)
        {
            RemoteCalendar cal = (RemoteCalendar)calendar;
